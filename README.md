...
Четыре свежих upstream-исправления PS/U-APSD уменьшили сбои, а на первом
LT300 v3 профиль выдержал четыре телефона после снижения `txpower` с 20 до
**15 dBm**. В том тесте была установлена маска MCS0–7, но её необходимость
отдельно не доказана. На втором LT300 v3 пользователь сообщил о нормальной
работе без маски при WPA2 и 12 dBm. Быстрая установка теперь не ставит маску
автоматически. Базовый профиль:

- HT20, корректный country code, фиксированный канал;
- U-APSD off, `disassoc_low_ack=0`, DTIM=1;
- AP TX MCS0–7 только как необязательный сравнительный тест;
- patched `mt76.ko`/`mt7603e.ko`;
- 15 dBm по умолчанию; 12 dBm для экземпляра с сохраняющимися обрывами
  (само снижение до 12 dBm на втором роутере проблему не устранило).
