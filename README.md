...
Воспроизводимый комплект для **Cudy LT300 v3** с OpenWrt 25.12.5: LTE через
специальный APN используется как транспорт, а весь клиентский трафик выходит в
...
- SSH к роутеру разрешён из `10.8.1.0/24` и `10.8.2.0/23`, но не открыт в LTE/WAN;
...
- Автоматический failover намеренно выключен. Переключение ручное и проверяемое.
