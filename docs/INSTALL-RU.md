...
/tmp/Cudy-LT300-OpenWrt-DualVPN-Kit/scripts/install-dual-vpn-switch.sh \
  wg0 awg0 wwan wg 10.10.11.192 10.8.1.0/24 10.8.2.0/23
```
...
`10.8.1.0/24` и AmneziaWG `10.8.2.0/23`. Для них создаются отдельные маршруты через
...
роутера, например `ssh root@10.8.1.3`.
...
Подтверждённый на этом LT300 v3 профиль оставляет HT20/WMM/802.11n, ограничивает передачу точки доступа индексами MCS0–7, отключает исключение клиента при low ACK, использует DTIM=1 и снижает мощность с 20 до 15 dBm. Дополнительно установлены исправленные `mt76.ko`/`mt7603e.ko` с четырьмя upstream PS/U-APSD-патчами. Четыре телефона одновременно выдержали реальную интернет-нагрузку; после reboot Wi-Fi, LTE и VPN поднялись автоматически.
...
- автоматическая маска AP TX MCS0–7 из `scripts/wifi-mcs-limit*`;
- без SQM до завершения диагностики.

Установка постоянной маски:
...
После `wifi down radio0; wifi up radio0` или reboot должна появиться новая строка `Applied HT MCS0-7 TX mask to phy0-ap0`. Скрипт ждёт `hostapd status=ENABLED`, потому что ранняя маска может быть сброшена во время запуска AP. Проверяйте фактическую скорость каждого клиента:

```sh
iw dev phy0-ap0 station dump
```

Одних MCS0–7, `disassoc_low_ack=0` и DTIM=1 оказалось недостаточно: на 20 dBm драйвер накопил более 100 `Beacon stuck`, `RX PSE busy`, тысячи повторов и очередь из 1077 пакетов. Штатный TX-hang reset очистил очередь. После установки исправленных модулей и снижения мощности до 15 dBm четыре клиента одновременно выдержали реальную нагрузку без обрыва. На другом экземпляре всё равно выполните собственный длительный тест.
...
Если Wi-Fi виден, но трафик зависает, смотрите station retries/failed и процессы в состоянии `D`. Убедитесь, что `logread | grep wifi-mcs-limit` подтверждает применение маски, а AP передаёт не выше MCS7. Проверяйте также питание, температуру и канал; исключите SQM и автоматические recovery-циклы. Не выгружайте `mt7603e`: зависший `rmmod` может остановить shutdown/sysupgrade и потребовать цикл питания.
