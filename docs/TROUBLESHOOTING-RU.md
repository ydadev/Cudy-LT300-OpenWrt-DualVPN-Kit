...
не в DHCP или VPN. На первом экземпляре помогли точные patched-модули плюс профиль HT20,
15 dBm, U-APSD off и DTIM 1; MCS0–7 был включён во время успешного теста, но
необходимость маски отдельно не доказана. Подробности и контрольные хеши находятся в
`WIFI-MT7603-RU.md`.
...
`iw`, но одно снижение мощности не убрало исчезновение SSID. Затем был
отключён автозапуск MCS0–7 и очищена маска через `iw dev phy0-ap0 set bitrates`;
пользователь сообщил о нормальной работе. Проверьте длительную нагрузку и reboot,
а также уменьшение радиуса покрытия. Сравнивайте счётчики Wi-Fi, не только
